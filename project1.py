...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    x = np.matmul(theta,feature_vector) + theta_0
    z = np.multiply(x,label)
    loss=np.max([0.0, (1-z)])
    return loss
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    for i in range(num):
        total.append(hinge_loss_single(feature_matrix[i], labels[i], theta, theta_0))
    hinge=np.average(total)
    return float(hinge)
    raise NotImplementedError
...
def perceptron_single_step_update(
    ...
    for i in range(0,n):
        x=(np.matmul(current_theta,feature_vector))+current_theta_0
        y=np.multiply(label,x)
        if y<=0:
